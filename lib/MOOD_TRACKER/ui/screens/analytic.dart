...
import '../../ui/widgets/chip_text.dart';
...
class _AnalyticScreenState extends State<AnalyticScreen> {
  ...
  Future<void> getBadEmotions() async {
    final result = await DbService.getBadMoodRelatedAttributeByCategory(
        AttributeCategory.Emotions);
    setState(() {
  ...
  Future<void> getGoodEmotions() async {
    final result = await DbService.getGoodMoodRelatedAttributeByCategory(
        AttributeCategory.Emotions);
    setState(() {
  ...
  Widget getHistorybottomTitles(double value, TitleMeta meta) {
    ...
    final currentDate = startDate.add(Duration(days: value.toInt()));
    return Text(DateFormat('dd/MM').format(currentDate), style: style);
  }
                        ...
                        LineChartBarData(
                          ...
                          dotData: FlDotData(
                              ...
                              getDotPainter: (spot, percent, barData, index) {
                                Color color =
                                    MoodValue.byNearestDoubleValue(spot.y)
                                            ?.color ??
                                        AppColors.gray500;
                  ...
                  child: BarChart(
                    BarChartData(
                        ...
                        touchTooltipData: BarTouchTooltipData(
                          ...
                          getTooltipItem: (
                            ...
                            final MoodValue? mood =
                                MoodValue.byNearestDoubleValue(rod.toY);
                      ...
                      barGroups: List.generate(
                          7,
                          (index) => getDowGroupData(
                              index, dowSummaryMap[index + 1] as double)),
                      gridData: FlGridData(
                        show: true,
                        drawVerticalLine: false,
                        getDrawingHorizontalLine: (double value) {
                          final MoodValue? mood =
                              MoodValue.byNearestDoubleValue(value + 1);
                          return FlLine(
